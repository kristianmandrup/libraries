...
(function(){
  var FileShortener, DirShortener, RootPath, isBlank, PathNormalizer, toString$ = {}.toString;
  ...
  module.exports = PathNormalizer = (function(){
    ...
    prototype.normalizeFilesFor = function(key, config){
      ...
      if (!isBlank(root)) {
        config.dir = root;
        return this.config[key] = this.fileShortener(config).shortenFiles();
      }
    };
    prototype.normalizeDirFor = function(key, config){
      ...
      return this.config[key] = this.dirShortener(config).shortenDir(this.root);
    };
    prototype.rootPathOf = function(files){
      return new RootPath(files);
    };
    prototype.fileShortener = function(config){
      config || (config = this.config);
      return new FileShortener(config);
    };
    prototype.dirShortener = function(config){
      config || (config = this.config);
      return new DirShortener(config);
    };
